   ...
   agent any
    ...
    stages {
        ...
        stage ("Download private key and tag from aws and store in hosts file") {
            steps {
                sh ("sudo aws ec2 describe-instances --filters 'Name=instance-state-name,Values=running' 'Name=tag:Name,Values=${workspace}' --query 'Reservations[].Instances[].[Tags[?Key==`Name`].Value[]]' --output text | sed -e '1s/^/[/' -e 's/$5/]/' >>/home/ubuntu/hosts")
